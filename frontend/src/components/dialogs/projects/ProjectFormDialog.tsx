...
export const ProjectFormDialog = NiceModal.create<ProjectFormDialogProps>(
  ({ project }) => {
    ...
    // Default task instructions appended to prompts
    const [appendPrompt, setAppendPrompt] = useState((project as any)?.append_prompt ?? '');
    ...
    useEffect(() => {
      if (project) {
        ...
        setAppendPrompt((project as any)?.append_prompt ?? '');
      } else {
        setName('');
        setGitRepoPath('');
        setSetupScript('');
      ...
      try {
        ...
        if (isEditing) {
          const updateData: UpdateProject = {
            ...
            // @ts-expect-error: field generated by backend; run `npm run generate-types` after syncing
